...
/**
 * Funções auxiliares globais para compatibilidade com cron e sync-manager
 */

/**
 * Importa categorias principais - função auxiliar para cron
 */
function art_image_import_categories() {
    ...
    do {
        $result = $importer->import_categories($page);
        $page++;
        
        // Log básico para acompanhar progresso
        if (!empty($result['logs'])) {
            foreach ($result['logs'] as $log) {
                error_log("[" . current_time('Y-m-d H:i:s') . "] " . $log);
            }
        }
...
/**
 * Importa subcategorias - função auxiliar para cron
 */
function art_image_import_subcategories() {
    $importer = new ArtImageImporter();
    $page = 1;
    
    do {
        $result = $importer->import_subcategories($page);
        $page++;
...
/**
 * Importa produtos - função auxiliar para cron
 */
function art_image_import_products() {
    $importer = new ArtImageImporter();
    $page = 1;
    $batch_size = 3; // Menor batch size para cron
...
/**
 * Importa artistas - função auxiliar para cron
 */
function art_image_import_artists() {
    ...
    do {
        $result = $importer->import_artists($page);
        $page++;
